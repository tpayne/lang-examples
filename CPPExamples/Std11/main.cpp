...
bool testCondition(testClass test) { return (test.iUid() > 500); }
...
void insertsClassIntoSet() {
  ...
  for (std::set<testClass>::const_iterator it2 = std::find_if(
           testClassesSet3.begin(), testClassesSet3.end(), testCondition);
       it2 != testClassesSet3.end(); ++it2) {
    std::cout << "The diff set test has iUid() = " << (*it2).iUid()
              << std::endl;
  }
...
void insertHistory() {
  ...
  std::vector<TestFuncs::history>::const_iterator it(
      changeSet.getHistory().begin());
  for (; it != changeSet.getHistory().end(); ++it) {
    std::cout << "History " << (*it).date() << " " << (*it).author() << " "
              << (*it).comment() << std::endl;
  }
}
...
void staticTest() {
  ...
  test.iUid(0);
  test1.iUid(-10);
  std::cout << "Number of dataClasses = " << TestFuncs::dataClass::num()
            << std::endl;
}
...
void testFindExamples98() {
  ...
  std::cout << "If all returned " << findExamples::if_all(testSet, pred)
            << std::endl;
  std::cout << "If any returned " << findExamples::if_any(testSet, pred)
            << std::endl;

  std::cout << "If all returned " << findExamples::if_all(testSet, test)
            << std::endl;
  std::cout << "If any returned " << findExamples::if_any(testSet, test)
            << std::endl;
...
void testSortExamples() {
  ...
  std::vector<std::string> strVec1 = {"be afraid", "hello", "hello1",
                                      "hello2"}; // c++11 only
  ...
  std::cout << "Are sorted reversed? " << sortExamples::isSorted(noVec)
            << std::endl;
  std::cout << "Are sorted reversed? " << sortExamples::isSorted(strVec)
            << std::endl;
  ...
  std::cout << "Are partially alike? " << sortExamples::isAlike(noVec1, noVec1)
            << std::endl;
  std::cout << "Are partially alike? " << sortExamples::isAlike(noVec1, noVec)
            << std::endl;
  std::cout << "Are partially alike? " << sortExamples::isAlike(noVec, noVec1)
            << std::endl;
