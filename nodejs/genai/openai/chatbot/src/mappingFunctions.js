 ...
 * @property {string[]} [waypoints] - An array of intermediate locations to include in the route.
 * @property {string} [mode] - Specifies the mode of transport. (e.g., "driving", "walking", "bicycling", "transit")
 * @property {string} [language] - The language to use for the results.
 * @property {string} [units] - Specifies the unit system to use. (e.g., "metric", "imperial")
 * @property {boolean} [alternatives] - If true, more than one route may be returned.
 * @property {string} [avoid] - Indicates features to avoid. (e.g., "tolls", "highways", "ferries", "indoor")
 * @property {string} [transit_mode] - Specifies the desired modes of transit. (e.g., "bus", "subway", "train", "tram", "rail")
 * @property {string} [transit_routing_preference] - Specifies preferences for transit routes. (e.g., "less_walking", "fewer_transfers")
 * @property {string} [departure_time] - The desired time of departure. Can be a timestamp or "now".
 * @property {string} [arrival_time] - The desired time of arrival (for transit). Can be a timestamp.
 * @property {string} [traffic_model] - Specifies the assumptions to use when calculating time in traffic. (e.g., "best_guess", "optimistic", "pessimistic")
 * @property {boolean} [optimizeWaypoints] - If true and waypoints are provided, the API will attempt to reorder the waypoints to minimize the total travel time.
 */
 ...
 * @async
 * @function planRoute
 * @param {string} sessionId - The unique identifier for the session (if needed for logging or context).
 * @param {DirectionsParameters} params - An object containing the parameters for the directions request.
 * @returns {Promise<object|null>} A promise that resolves to the route data (object) in JSON format,
 * or null if the route cannot be planned.
 * @throws {Error} If there is an error during the API call or if the API key is missing.
 */
