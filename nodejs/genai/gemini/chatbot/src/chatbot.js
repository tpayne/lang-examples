...
const getChatResponse = async (userInput, forceJson = false) => {
    ...
    try {
        ...
        const model = ai.models.get(getConfig().aiModel);
        const generationConfig = {
            maxOutputTokens: Number(getConfig().maxTokens),
            temperature: 1,
            topP: 1,
        };
        const parts = [{ text: `${ctxStr}\n${contxtStr}` }];
        ...
        while (!finalResponse && numFunctionCalls < maxFunctionCalls) {
            const result = await model.generateContent({
                contents: parts,
                generationConfig,
                tools: toolsConfig.tools.length > 0 ? toolsConfig : undefined,
            });
            ...
            if (response.candidates && response.candidates.length > 0) {
                ...
                break; // Exit the loop if no candidates are found
            }
        ...
        if (!finalResponse && functionCallResult !== null) {
            finalResponse = functionCallResult; // If the last action was a function call and no final text response was generated
        }
