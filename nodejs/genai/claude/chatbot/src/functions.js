...
const { getDockerImageTags, searchDockerImages } = require('./dockerHub'); // Import the new dockerhub.js module
...
const { scanNetworkForSSH } = require('./sshscan'); // Import the SSH scanning function
...
async function registerFunction(
  ...
  try {
    if (sessionRegistry[name]) {
      ...
      // Anthropic Messages API tool format. Unlike OpenAI's
      // { type: 'function', function: { name, description, parameters } },
      // Claude tools are flat: { name, description, input_schema }.
...
async function loadCodeReviews(sessionId) {
  ...
  await registerFunction(
    ...
    'Review files in a given GitHub repository. Use this for GitHub URLs and GitHub repositories.',
    {
      ...
      repoDirName: {
        type: 'string',
        description: 'The GitHub repository path to start download at.',
      },
      branchName: {
        type: 'string',
        description: 'The name of the branch to review.',
      },
    },
...
async function loadAdoCodeReviews(sessionId) {
  ...
  await registerFunction(
    ...
    'Review files in a given Azure DevOps repository. Use this for Azure DevOps URLs and Azure DevOps repositories.',
    {
      ...
      repoDirName: {
        type: 'string',
        description: 'The Azure DevOps repository path to start download at.',
      },
      branchName: {
        type: 'string',
        description: 'The name of the branch to review.',
...
async function loadDosa(sessionId) {
  await registerFunction(
    ...
    'Get the MOT History for a vehicle.',
    {
      registrationNumber: {
        type: 'string',
        description: 'The Vehicle registration or VIN number.',
      },
    },
    ['registrationNumber'],
    true,
...
async function loadGitHub(sessionId) {
  await registerFunction(
    ...
    'Create a GitHub repository under an organisation or user',
    {
      repoName: {
        type: 'string',
        description:
          'The name of the repository where the branch will be created.',
      },
      username: {
        type: 'string',
      ...
      description: {
        type: 'string',
        description:
          'The description of the repository name (optional). Defaults if not provided',
      },
      isPrivate: {
        type: 'boolean',
        description:
          'Is the repository private (true) or public (false) (optional). Defaults to public if not provided',
      },
    },
    ['repoName'],
  );
  ...
  await registerFunction(
    ...
    'Save generated code or changes made by the bot to a local file',
    {
      code: {
        type: 'string',
        description: 'The generated code or changes to save.',
      },
      filename: {
        type: 'string',
        description: 'The local filename to save the changes to.',
      },
      repoDirName: {
        type: 'string',
        description: 'The directory name that is used (optional).',
      },
    },
    ['code', 'filename', 'repoDirName'],
    true,
  ...
  await registerFunction(
    ...
    'Create a new branch in a GitHub repository based on an existing branch',
    {
      ...
      baseBranch: {
        type: 'string',
        description:
          'The name of the existing branch to base the new branch on (optional). Defaults to "main".',
      },
    },
    ['username', 'repoName', 'branchName'],
  );
  ...
  // Register function for GitHub getFileContents
  await registerFunction(
    sessionId,
    'get_github_file_contents',
    getFileContents,
    ['username', 'repoName', 'filePath', 'branchName'],
    'Gets the contents of a single file from a GitHub repository.',
    {
      ...
      branchName: {
        type: 'string',
        description:
          'Optional branch name. If not specified, the default branch will be used.',
      },
    },
  ...
  await registerFunction(
    ...
    'Upload, push, load or commit files from the session temporary directory to a specified GitHub repository, maintaining directory structure.', // Updated description if needed
    {
      ...
      repoDirName: {
        type: 'string',
        description:
          'The base directory in the GitHub repository to commit files into.',
  ...
  await registerFunction(
    ...
    'Lists the contents of a directory in a GitHub repository on a specific branch. Defaults to the root and recursive scan.', // Updated description
    {
      ...
      branchName: {
        type: 'string',
        description:
          'The name of the branch to list contents from (optional). Defaults to "main" if not provided.',
      }, // Added branch parameter definition
      repoDirName: {
        type: 'string',
        description:
          'The directory path within the repository (optional). Defaults to the repository root if not provided.',
      }, // Clarified description
      recursive: {
        type: 'boolean',
        description:
          'Perform a recursive scan or not (optional). Defaults to true if not provided.',
      }, // Clarified description
    },
...
async function loadGitHubActions(sessionId) {
  ...
  await registerFunction(
    ...
    'Lists the GitHub actions running in a GitHub repository.',
    {
      ...
      status: {
        type: 'string',
        description:
          'The status of the actions (optional). Defaults to in_progress if not provided',
      },
    },
    ['username', 'repoName'],
  );
  ...
  await registerFunction(
    ...
    'Dispatches a workflow_dispatch event to trigger a workflow.',
    {
      ...
      ref: {
        type: 'string',
        description: 'The Git reference (branch, tag, or SHA).',
      },
      inputs: {
        type: 'object',
        description: 'The inputs for the workflow_dispatch event.',
      },
    },
...
async function loadAdoIntegration(sessionId) {
  await registerFunction(
    ...
    'Create an Azure DevOps (ADO) Git repository under a specified organization and project.',
    {
      ...
      isPublic: {
        type: 'boolean',
        description:
          'Is the repository public (true) or private (false) (optional). Defaults to private if not provided.',
      },
    },
    ['organization', 'project', 'repoName'],
  );

  // Register function for Azure DevOps getAdoFileContents
  await registerFunction(
    sessionId,
    'get_ado_file_contents',
    getAdoFileContents,
    ['organization', 'project', 'repoName', 'filePath', 'branchName'],
    ...
    ['organization', 'project', 'repoName', 'filePath'],
  );
  ...
  await registerFunction(
    ...
    'Switch the default branch in an Azure DevOps (ADO) Git repository.',
    {
      ...
      branchName: {
        type: 'string',
        description: 'The branch name to switch to as the new default.',
      },
    },
    ['organization', 'project', 'repoName', 'branchName'],
  );
  ...
  await registerFunction(
    ...
    'Upload, push, load or commit files from the session temporary directory to a specified Azure DevOps (ADO) Git repository, maintaining directory structure.',
    {
      ...
      repoDirName: {
        type: 'string',
        description:
          'The base directory in the Azure DevOps repository to commit files into (optional). Defaults to the repository root if not provided.',
      },
      branchName: {
        type: 'string',
        description:
          "The name of the branch to commit to (optional). Defaults to the repository's default branch if not provided.",
      },
    },
    ['organization', 'project', 'repoName'],
    true,
  ...
  await registerFunction(
    ...
    'Create a pull request on a given Azure DevOps (ADO) Git repository.',
    {
      ...
      body: {
        type: 'string',
        description: 'The description or body of the pull request (optional).',
      },
    },
    [
      'organization',
  ...
  await registerFunction(
    ...
    'Fetch or download the contents of an Azure DevOps (ADO) Git repository.',
    {
      ...
      repoDirName: {
        type: 'string',
        description:
          'The Azure DevOps repository path to start download at (optional). Defaults to the repository root if not provided.',
      },
      branchName: {
        type: 'string',
        description:
          "The name of the branch to fetch contents from (optional). Defaults to the repository's default branch if not provided.",
      },
    },
    ['organization', 'project', 'repoName'],
    true,
  ...
  await registerFunction(
    ...
    'Lists Azure DevOps (ADO) Pipelines and builds that ran in a specified repository.',
    {
      ...
      statusFilter: {
        type: 'string',
        description:
          'The status to filter pipelines by (e.g., "inProgress", "queued", "completed", "failed") (optional). Defaults to "inProgress" if not provided.',
      },
    },
    ['organization', 'project', 'repoName'],
  );
  ...
  await registerFunction(
    ...
    'Lists the contents of a directory in an Azure DevOps (ADO) Git repository on a specific branch. Defaults to the root and recursive scan.',
    {
      ...
      branchName: {
        type: 'string',
        description:
          "The name of the branch to list contents from (optional). Defaults to the repository's default branch if not provided.",
      },
      repoDirName: {
        type: 'string',
        description:
  ...
  await registerFunction(
    ...
    'Create a new branch in an Azure DevOps (ADO) Git repository based on an existing branch.',
    {
      ...
      baseBranch: {
        type: 'string',
        description:
          "The name of the existing branch to base the new branch on (optional). Defaults to the repository's default branch.",
      },
    },
    ['organization', 'project', 'repoName', 'branchName'],
  );
  ...
  await registerFunction(
    ...
    'Creates a new Azure DevOps pipeline (build definition).',
    {
      ...
      yamlPath: {
        type: 'string',
        description:
          'The path to the YAML file defining the pipeline (default is "azure-pipelines.yml".',
      },
    },
    ['organization', 'project', 'repoName', 'yamlPath'],
  );
  ...
  await registerFunction(
    ...
    'Triggers a new run for an Azure DevOps pipeline.',
    {
      ...
      repoName: {
        type: 'string',
        description:
          'The name of the repository where the pipeline will be run.',
      },
      branchName: { type: 'string', description: 'The name of branch to use.' },
      userParameters: {
        type: 'object',
        description:
          'Optional parameters for the pipeline run (e.g., variables, branch).',
      },
    },
  ...
  await registerFunction(
    ...
    'Fetches a list of runs for a specific Azure DevOps pipeline.',
    {
      ...
      project: {
        type: 'string',
        description: 'The Azure DevOps project ID or name.',
      },
      pipelineId: { type: 'number', description: 'The ID of the pipeline.' },
    },
    ['organization', 'project', 'pipelineId'],
    ...
    'Plans a route between multiple points using the Google Maps Directions API and returns the details in JSON format.', // Description for the AI
    {
      ...
      params: {
        ...
        properties: {
          origin: {
            type: 'string',
            description: 'The starting point for the directions request.',
          },
          destination: {
            type: 'string',
            description: 'The ending point for the directions request.',
          },
          waypoints: {
            type: 'array',
            description:
              'An array of intermediate locations to include in the route.',
            items: { type: 'string' },
          ...
          mode: {
            type: 'string',
            description:
              'Specifies the mode of transport (e.g., "driving", "walking", "bicycling", "transit"). Defaults to "driving".',
            nullable: true, // Make optional
            enum: ['driving', 'walking', 'bicycling', 'transit'], // Add supported modes
          },
          language: {
            type: 'string',
            description: 'The language to use for the results.',
            nullable: true,
          }, // Make optional
          units: {
            type: 'string',
            description:
              'Specifies the unit system to use (e.g., "metric", "imperial"). Defaults to "metric".',
            nullable: true, // Make optional
            enum: ['metric', 'imperial'], // Add supported units
          },
          alternatives: {
            type: 'boolean',
            description: 'If true, more than one route may be returned.',
            nullable: true,
          }, // Make optional
          avoid: {
            type: 'string',
            description:
              'Indicates features to avoid (e.g., "tolls", "highways", "ferries", "indoor"). Can be a single value or multiple values separated by "|".',
            nullable: true, // Make optional
          },
          transit_mode: {
            type: 'string',
            description:
              'Specifies the desired modes of transit (e.g., "bus", "subway", "train", "tram", "rail"). Can be multiple values separated by "|".',
            nullable: true, // Make optional
          },
          transit_routing_preference: {
            type: 'string',
            description:
              'Specifies preferences for transit routes (e.g., "less_walking", "fewer_transfers").',
            nullable: true, // Make optional
          ...
          departure_time: {
            type: 'string',
            description:
              'The desired time of departure. Can be a timestamp or "now".',
            nullable: true,
          }, // Make optional
          arrival_time: {
            type: 'string',
            description:
              'The desired time of arrival (for transit). Can be a timestamp.',
            nullable: true,
          }, // Make optional
          traffic_model: {
            type: 'string',
            description:
              'Specifies the assumptions to use when calculating time in traffic (e.g., "best_guess", "optimistic", "pessimistic").',
            nullable: true, // Make optional
          ...
          optimizeWaypoints: {
            type: 'boolean',
            description:
              'If true and waypoints are provided, the API will attempt to reorder the waypoints to minimize the total travel time.',
            nullable: true,
          }, // Make optional
        },
      ...
      params: {
        ...
        properties: {
          ...
          waypoints: {
            type: 'array',
            description:
              'An array of intermediate locations included in the route.',
            items: { type: 'string' },
...
async function loadKubernetes(sessionId) {
  ...
  await registerFunction(
    ...
    'Create a Kubernetes resource from a provided YAML or JSON body.',
    {
      resourceType: {
        type: 'string',
        description:
          'The type of the resource to create (e.g., "pods", "deployments", "services", "namespaces").',
      },
      resourceBody: {
        type: 'string',
        description:
          'The YAML or JSON string representation of the Kubernetes resource to create. This should be auto-generated by the chatbot based on the description given',
      },
      namespace: {
        type: 'string',
        description:
          'Optional: The namespace in which to create the resource. Not required for cluster-scoped resources like Namespaces or PersistentVolumes. The default will be "default" if not provided.',
      },
    },
  ...
  await registerFunction(
    ...
    'Delete a specified Kubernetes resource (e.g., Pod, Deployment, Service) by its name from a given namespace.',
    {
      resourceType: {
        type: 'string',
        description:
          'The type of the resource to delete (e.g., "pods", "deployments", "services", "namespaces").',
      },
      name: {
        type: 'string',
      ...
      namespace: {
        type: 'string',
        description:
          'The namespace the resource resides in. Required for namespaced resources. Not used for cluster-scoped resources like PersistentVolumes or Namespaces (when deleting the namespace itself).',
      },
    },
    ['resourceType', 'name'],
    true,
  ...
  await registerFunction(
    ...
    'Update an existing Kubernetes resource with a provided YAML or JSON body.',
    {
      resourceType: {
        type: 'string',
        description:
          'The type of the resource to update (e.g., "pods", "deployments", "services", "namespaces").',
      },
      name: {
        type: 'string',
      ...
      resourceBody: {
        type: 'string',
        description:
          'The YAML or JSON string representation of the Kubernetes resource with the updated configuration. This should be auto-generated by the chatbot based on the description given',
      },
      namespace: {
        type: 'string',
        description:
  ...
  await registerFunction(
    ...
    'List or get all the Kubernetes deployments across all namespaces or within a specified namespace, returning their detailed status, replicas, and associated metadata.',
    {
      namespace: {
        type: 'string',
        description:
          'Optional: The namespace to list deployments from. If not provided, lists from all namespaces.',
      },
    },
  ...
  await registerFunction(
    ...
    'Get detailed information about a specific Kubernetes Deployment by its name within a given namespace.',
    {
      deploymentName: {
        type: 'string',
        description: 'The name of the deployment to get details for.',
      },
      namespace: {
        type: 'string',
  ...
  await registerFunction(
    ...
    'List or get all the Kubernetes services across all namespaces or within a specified namespace, returning their detailed status, cluster IPs, ports, and selectors.',
    {
      namespace: {
        type: 'string',
        description:
          'Optional: The namespace to list services from. If not provided, lists from all namespaces.',
      },
    },
    [], // No required parameters, namespace is optional
    true, // needSession is true
  ...
  await registerFunction(
    ...
    'Get detailed information about a specific Kubernetes Service by its name within a given namespace.',
    {
      serviceName: {
        type: 'string',
        description: 'The name of the service to get details for.',
      },
      namespace: {
        type: 'string',
        description: 'The namespace the service resides in.',
      },
    },
    ['serviceName', 'namespace'],
    true,
  ...
  await registerFunction(
    ...
    'List or get all the Kubernetes pods across all namespaces or within a specified namespace, returning their detailed status, node assignments, and container information.',
    {
      namespace: {
        type: 'string',
        description:
          'Optional: The namespace to list pods from. If not provided, lists from all namespaces.',
      },
    },
    [], // No required parameters, namespace is optional
    true, // needSession is true
  ...
  await registerFunction(
    ...
    'Get detailed information about a specific Kubernetes Pod by its name within a given namespace. This is ONLY for Kubernetes use, not for host machine details',
    {
      ...
      namespace: {
        type: 'string',
        description: 'The namespace the pod resides in.',
      },
    },
    ['podName', 'namespace'],
    true,
  ...
  await registerFunction(
    ...
    'Get detailed information about a specific Kubernetes Secret by its name within a given namespace.',
    {
      secretName: {
        type: 'string',
        description: 'The name of the secret to get details for.',
      },
      namespace: {
        type: 'string',
        description: 'The namespace the secret resides in.',
      },
    },
    ['secretName', 'namespace'],
    true,
  ...
  await registerFunction(
    ...
    'List or get all Kubernetes Events across all namespaces or within a specified namespace, returning their detailed status and associated resources.',
    {
      namespace: {
        type: 'string',
        description:
          'Optional: The namespace to list events from. If not provided, lists from all namespaces.',
      },
    },
    [], // No required parameters, namespace is optional
    true,
  ...
  await registerFunction(
    ...
    'List or get all Kubernetes Secrets across all namespaces or within a specified namespace, returning their detailed data (excluding sensitive content).',
    {
      namespace: {
        type: 'string',
        description:
          'Optional: The namespace to list secrets from. If not provided, lists from all namespaces.',
      },
    },
    [], // No required parameters, namespace is optional
    true,
  ...
  await registerFunction(
    ...
    'List or get all the Kubernetes ReplicaSets across all namespaces or within a specified namespace, returning their detailed status and replica counts.',
    {
      namespace: {
        type: 'string',
        description:
          'Optional: The namespace to list ReplicaSets from. If not provided, lists from all namespaces.',
      },
    },
    [], // No required parameters, namespace is optional
    true, // needSession is true
  ...
  await registerFunction(
    ...
    'List or get all the Kubernetes DaemonSets across all namespaces or within a specified namespace, returning their detailed status.',
    {
      namespace: {
        type: 'string',
        description:
          'Optional: The namespace to list DaemonSets from. If not provided, lists from all namespaces.',
      },
    },
    [], // No required parameters, namespace is optional
    true, // needSession is true
  ...
  await registerFunction(
    ...
    'List or get all the Kubernetes StatefulSets across all namespaces or within a specified namespace, returning their detailed status and replica counts.',
    {
      namespace: {
        type: 'string',
        description:
          'Optional: The namespace to list StatefulSets from. If not provided, lists from all namespaces.',
      },
    },
    [], // No required parameters, namespace is optional
    true, // needSession is true
  ...
  await registerFunction(
    ...
    'List or get all the Kubernetes Ingresses across all namespaces or within a specified namespace, returning their detailed rules and backend services.',
    {
      namespace: {
        type: 'string',
        description:
          'Optional: The namespace to list Ingresses from. If not provided, lists from all namespaces.',
      },
    },
    [], // No required parameters, namespace is optional
    true, // needSession is true
  ...
  await registerFunction(
    ...
    'List or get all the Kubernetes ConfigMaps across all namespaces or within a specified namespace, returning their detailed data.',
    {
      namespace: {
        type: 'string',
        description:
          'Optional: The namespace to list ConfigMaps from. If not provided, lists from all namespaces.',
      },
    },
    [], // No required parameters, namespace is optional
    true, // needSession is true
  ...
  await registerFunction(
    ...
    'List or get all the Kubernetes PersistentVolumeClaims across all namespaces or within a specified namespace, returning their detailed status and volume bindings.',
    {
      namespace: {
        type: 'string',
        description:
          'Optional: The namespace to list PersistentVolumeClaims from. If not provided, lists from all namespaces.',
      },
    },
    [], // No required parameters, namespace is optional
    true, // needSession is true
  ...
  await registerFunction(
    ...
    'List or get all the Kubernetes Jobs across all namespaces or within a specified namespace, returning their detailed status and completion information.',
    {
      namespace: {
        type: 'string',
        description:
          'Optional: The namespace to list Jobs from. If not provided, lists from all namespaces.',
      },
    },
    [], // No required parameters, namespace is optional
    true, // needSession is true
  ...
  await registerFunction(
    ...
    'List or get all the Kubernetes CronJobs across all namespaces or within a specified namespace, returning their detailed status and schedule.',
    {
      namespace: {
        type: 'string',
        description:
          'Optional: The namespace to list CronJobs from. If not provided, lists from all namespaces.',
      },
    },
    [], // No required parameters, namespace is optional
    true, // needSession is true
  ...
  await registerFunction(
    ...
    'Search for Docker images on Docker Hub, supporting both public and private repositories. For private searches, the Docker Hub username and password must be configured as environment variables.',
    {
      searchTerm: {
        type: 'string',
        description:
          'The term to search for (e.g., "ubuntu", "my-private-repo/my-image").',
      },
      isPrivate: {
        type: 'boolean',
        description:
          'Set to true to search private repositories of the configured Docker Hub user. Defaults to false (public search).',
      },
      username: {
        type: 'string',
        description:
          'Optional: Docker Hub username if searching private repos. Defaults to DOCKERHUB_USERNAME env var if not provided and isPrivate is true.',
      },
    },
  ...
  await registerFunction(
    ...
    'For a given Docker image name, get all the details of its tags.',
    {
      imageName: {
        type: 'string',
        description:
          'The Docker image name to get tags for (e.g., "latest", "my-private-repo/my-image").',
      },
    },
    ['imageName'],
  );
...
async function loadDatabaseFunctions(sessionId) {
  await registerFunction(
    ...
    'Connects to a database using a JDBC-like URI. This function establishes a connection to the specified database and returns a client object for further operations.',
    {
      uri: {
        type: 'string',
        description:
          'The JDBC-like URI for the database connection (e.g., "jdbc:postgresql://host:port/database?user=...").',
      },
    },
    ['uri'],
    true,
  ...
  await registerFunction(
    ...
    'Dumps the structure (tables, views, columns) of a database in JSON format. Requires a JDBC-like URI to connect.',
    {
      uri: {
        type: 'string',
        description: 'The JDBC-like URI for the database connection.',
      },
    },
    ['uri'],
    true,
  ...
  await registerFunction(
    ...
    'Selects an optional percentage of data from a specified table or view in a database, returning the results in JSON format including column names. Defaults to 10% if percentage is not specified.',
    {
      ...
      tableNameOrViewName: {
        type: 'string',
        description: 'The name of the table or view to select data from.',
      },
      percentage: {
        type: 'number',
        description:
          'Optional: The percentage of data to select (0-100). Defaults to 10% if not provided.',
      },
    },
  ...
  await registerFunction(
    ...
    'Lists objects (tables, views, indexes, constraints) owned by a specific schema in a database. The type of objects to list can be specified, defaulting to all if not provided.',
    {
      ...
      schemaName: {
        type: 'string',
        description: 'The name of the schema to list objects from.',
      },
      objectTypes: {
        type: 'array',
        items: {
          type: 'string',
          enum: ['tables', 'views', 'indexes', 'constraints'],
        },
        description:
          'Optional: An array of object types to list (e.g., ["tables", "views"]). Defaults to all types if not provided.',
      },
    },
    ['uri', 'schemaName'],
    true,
  ...
  await registerFunction(
    ...
    'Executes any arbitrary SQL query against a connected relational database. This includes DML (INSERT, UPDATE, DELETE, SELECT) and DDL (CREATE, ALTER, DROP, etc.) statements, as well as stored procedure calls, triggers, and other advanced SQL commands. Returns query results for SELECT statements and affected rows for DML/DDL statements.',
    {
      ...
      sqlQuery: {
        type: 'string',
        description: 'The free-text SQL query to execute.',
      },
    },
    ['uri', 'sqlQuery'],
    true,
  ...
  await registerFunction(
    ...
    'Scans a network range for hosts with SSH (port 22 by default) open. Returns a list of reachable SSH targets in the format "ip:port". Useful for discovering available SSH endpoints on a subnet.',
    {
      baseIp: {
        type: 'string',
        description:
          'The base IP address of the subnet to scan (e.g., "192.168.1").',
      },
      startRange: {
        type: 'number',
        description:
          'The starting value for the last octet in the IP range (default: 1).',
      },
      endRange: {
        type: 'number',
        description:
          'The ending value for the last octet in the IP range (default: 254).',
      },
      port: {
        type: 'number',
      ...
      timeout: {
        type: 'number',
        description:
          'Timeout in milliseconds for each connection attempt (default: 1000).',
      },
    },
  ...
  await registerFunction(
    ...
    'Collects basic host machine information, including OS, CPU, memory and hardware details. Returns the data in a structured JSON format. Can target a remote host via SSH with the username and password looked up from a Docker secret map if the username is not specified. Defaults to the Docker host if no SSH target is provided or password is not found.',
    {
      ...
      sshTarget: {
        type: 'string',
        description:
          'Optional: The SSH target machine. If no user is specified it will be looked up. The password for this target will be looked up from a Docker secret map.',
      },
    },
...
async function loadIntegrations(sessionId) {
  if (process.env.GITHUB_TOKEN) {
    ...
    logger.info(
      `Loading GitHub code review integration for session: ${sessionId}`,
    );
  ...
  if (process.env.AZURE_DEVOPS_PAT) {
    logger.info(
      `Loading Azure DevOps (ADO) integration for session: ${sessionId}`,
    );
    await loadAdoIntegration(sessionId);
    logger.info(
      `Loading Azure DevOps (ADO) code review integration for session: ${sessionId}`,
    );
    await loadAdoCodeReviews(sessionId);
  } else {
    logger.info(
      `Azure DevOps (ADO) integration not loaded for session: ${sessionId}. AZURE_DEVOPS_PAT not set.`,
    );
  }

  if (
    process.env.DOSA_API_KEY
    && process.env.DOSA_API_SECRET
  ...
  if (
    process.env.KUBERNETES_API_ENDPOINT
    && process.env.KUBERNETES_BEARER_TOKEN
  ) {
    ...
    logger.info(
      `Kubernetes integration not loaded for session: ${sessionId}. KUBERNETES_API_ENDPOINT or KUBERNETES_BEARER_TOKEN not set.`,
    );
  }
  ...
  logger.info(
    `Loading System Information integration for session: ${sessionId}`,
  );
